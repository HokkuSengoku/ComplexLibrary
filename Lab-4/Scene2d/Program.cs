    ...
    internal class Program
        {
            internal static void Main(string[] args)
            {
                ...
                IEnumerable<string> commands =
                    readCommandsFromFile ? ReadCommandsFromFile(args[0]) : ReadCommandsFromUserInput();
                ...
                foreach (string commandLine in commands)
                {
                    ...
                    catch (BadNameException message)
                    {
                        Console.WriteLine($"{message.ExceptionMessage}");
                    }
                    catch (NameDoesAlreadyExistException message)
                    {
                        Console.WriteLine($"{message.ExceptionMessage}");
                    }
                    catch (BadCircleRadiusException message)
                    {
                        Console.WriteLine($"{message.ExceptionMessage}");
                    }
                    catch (BadRectanglePointException message)
                    {
                        Console.WriteLine($"{message.ExceptionMessage}");
                    }
                    catch (BadPolygonPointNumberException message)
                    {
                        Console.WriteLine($"{message.ExceptionMessage}");
                    }
                    catch (BadPolygonPointException message)
                    {
                        Console.WriteLine($"{message.ExceptionMessage}");
                    }
